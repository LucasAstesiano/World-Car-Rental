...
const ListUser=()=>{
    ...
    const url="http://localhost:8080/usuario/usuarios"

    const [userControler,setUserControler]= useState({})

    const email=localStorage.getItem('email')       
    
    const url2=`http://localhost:8080/usuario/buscarEmail?email=${email}`
            ...
            <table>
                ...
                {users.map((user, index) => (
                    <tbody key={index}>
                        ...
                        <td>{user.rol}</td>
                        <td>
                      <Link to={`/editUser/${user.id}`}><button className='btn-edit'>Editar</button></Link>
