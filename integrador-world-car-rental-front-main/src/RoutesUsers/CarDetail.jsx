...
const CarDetails = () => {
    ...
    useEffect(() => {        
      ...
      const handleSaveDates = () => {
        ...
        console.log("data del fetch:" + config.body);
        fetch("http://localhost:8080/reserva/reservar",config)
        .then((response)=>{
          return response
        })
        .then((data)=>{
        ...
        {vehicleDetails ? ( 
          <>
            {vehicleDetails.imagen && vehicleDetails.imagen.length > 0 ? (
              <img src={vehicleDetails.imagen[0]} alt="imagen vehiculo" style={{width:"60%", borderRadius:"12px"}}/> 
            ) : (
